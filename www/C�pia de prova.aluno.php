				...
				if ( is_object($perg)){
					...
					switch ($perg->getTipoPergunta()->getId()){
						...
						case "1";	$prova_resposta    = addslashes(trim($_POST['dissertativa_resposta_'.$prova_pergunta]));
									...
									#echo "<br>Pergunta: $prova_pergunta -> Resposta: $prova_resposta";
...
//									$prov->addResposta(null,$prova_pergunta, $pergunta_resposta, $pergunta_resposta);
						...
						/* Complete */
						case "4";	$pergunta_resposta = $_POST['complete_resposta_texto_'.$prova_pergunta];
									while (list ($key,$val) = @each ($pergunta_resposta)) {
										if (strlen($val)>0){
											//$prov->addResposta('',$prova_pergunta, $val,$val);
											#echo "<br>Pergunta: $prova_pergunta -> Resposta: $val ( $respostas )";
...
if (is_object($prov)){
	for ($j=0;$j<$prov->getQtdePerguntas();$j++){
		...
		if ($perg->getTipoPergunta()->getId()=="4"){
			...
			for ($i=0; $i<$perg->getQtdeResposta(); $i++){
				$model->assign_block_vars('pergunta.complete.item',array('RESPOSTA'			=>	$perg->getResposta($i)->getId(),
																		'RESPOSTA_TEXTO'	=>	$perg->getResposta($i)->getRespostaTexto(),
		...
		if ($perg->getTipoPergunta()->getId()=="5"){
			...
			for ($i=0; $i<$perg->getQtdeResposta(); $i++){
				$model->assign_block_vars('pergunta.lacuna.item_pergunta',array('PROVA_PERGUNTA'=>	$perg->getId(),
																		'PROVA_RESPOSTA'		=>	$perg->getResposta($i)->getId(),
																		'RESPOSTA_TEXTO'		=>	$perg->getResposta($i)->getRespostaTexto(),
				...
				$model->assign_block_vars('pergunta.lacuna.item_seta',array());
				$model->assign_block_vars('pergunta.lacuna.item_resposta',array('PROVA_PERGUNTA'=>	$perg->getId(),
																		'PROVA_RESPOSTA'		=>	$perg->getResposta($i)->getId(),
																		'RESPOSTA_TEXTO'		=>	$perg->getResposta($i)->getRespostaTexto(),
																		'RESPOSTA_TEXTO_FILHO'	=>	is_object($perg->getResposta($i)->getRespostaFilho())?$perg->getResposta($i)->getRespostaFilho()->getRespostaTexto():"",
																		'CLASSE'				=>  ($i%2==0)?"class='odd'":"",
