...
class ImagemDAO extends DAO {
	...
	public function gravaDadosImagem(Imagem $imagem){
		...
		if (strlen($imagem->getId())>0){
				$query = " UPDATE tbl_pergunta_imagem SET
				...
				$query = "INSERT INTO tbl_pergunta_imagem (
								pergunta,
								descricao,
								path,
								thumb
	...
	public function recuperarImagem($id_imagem){
		...
		$query ="SELECT tbl_pergunta_imagem.imagem       AS imagem,
						tbl_pergunta_imagem.pergunta     AS pergunta,
						tbl_pergunta_imagem.descricao    AS descricao,
						tbl_pergunta_imagem.path         AS path,
						tbl_pergunta_imagem.thumb        AS thumb
				FROM tbl_pergunta_imagem
				WHERE tbl_pergunta_imagem.imagem = ".$id_imagem;
	...
	public function recuperarTodos() {
		...
		$sql = "SELECT DISTINCT imagem
				...
				ORDER BY tbl_curso.nome ASC, tbl_disciplina.nome ASC, tbl_pergunta.descricao ASC, tbl_pergunta_imagem.path ASC"; 
		$banco = $this->getBancoDados(); 
	...
	public function recuperarTodosPergunta($pergunta) {
		...
		$sql = "SELECT imagem
				FROM tbl_pergunta_imagem 
				WHERE tbl_pergunta_imagem.pergunta  = ".$pergunta->getId()."
				ORDER BY tbl_pergunta_imagem.pergunta ASC"; 
		$retorno = $banco->executaSQL($sql);
		if($retorno != NULL) {
